...
const htmlfy = await import('htmlfy').then((htmlfy) =>
  Bun.env.NODE_ENV !== 'production' ? htmlfy.prettify : htmlfy.minify
)

export default function renderReactPage<
  T extends {locale: ALTERNATES_TYPE; pathname: string},
>(data: T, Node: (data: T) => ReactNode, reactUrl: string): Response {
  const html = renderToString(React.createElement(Node, data)).replaceAll(
